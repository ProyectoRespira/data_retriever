...
def combine_existing_and_new_readings(df1, df2, klogger):
    try:
        klogger.info('Combining existing and new readings.')
...
def drop_bad_readings(df, klogger):
    try:
        klogger.info('Dropping bad readings based on predefined thresholds.')
...
def interpolate_missing_data(df, klogger):
    try:
        klogger.info('Interpolating missing data.')
        df_interpolated = df
...
def set_variable_dtypes(df, klogger):
    try:
        klogger.info('Setting variable data types.')
        df = df.astype({
...
def process_weather_silver(group, klogger):
    try:
        klogger.info(f"Processing group for weather station: {group['weather_station'].iloc[0]}")
        ...
        klogger.info(f"Completed processing for weather station: {group['weather_station'].iloc[0]}")
...
def transform(data, data_2, *args, **kwargs):
    ...
    try:
        ...
        processed_data = group.groupby('weather_station').apply(process_weather_silver, klogger=klogger).reset_index(drop=True)
